...
export default function RoomsPage({
  ...
  async function handleReorder(reordered: Room[]) {
    setRooms(reordered);
    await fetch("/api/reorder", {
      ...
      body: JSON.stringify({
        model: "room",
        items: reordered.map((r, i) => ({ id: r.id, order: i })),
      }),
    });
  }
  ...
  return (
    <div className="min-h-dvh bg-slate-50 dark:bg-slate-950">
      <Header title={subjectName || "Rooms"} showBack reorderMode={reorderMode} onToggleReorder={() => setReorderMode((v) => !v)} />
          ...
          <SortableList
            ...
            renderItem={(room, i) => (
              <div
                ...
                onClick={() => !reorderMode && router.push(`/categories/${categoryId}/${subjectId}/${room.id}`)}
              >
